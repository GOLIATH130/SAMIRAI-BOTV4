...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=573175852207:+57 317 585 22 07\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = botPrefix
blocked = []
...
async function starts() {
	...
	await nzwa.connect({timeoutMs: 30*1000})
        ...
        console.log('=> Samurai bot v4!')
        lolcatjs.fromString('[Samurai] Hola gracias por usar mi bot, espero te guste ;)')
	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Joder @${num.split('@')[0]}\n Que bueno verte por aca😉  *${mdata.subject}* espero la pases super y recuerda leer las reglas para que no allá malos entendidos🥺`
				...
				teks = `Que bueno que te fuiste ya ni te queríamos 😎 @${num.split('@')[0]}ojala nunca regreses 👋`
				let buff = await getBuffer(ppimg)
				nzwa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
              ...
              await nzwa.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Usa el comando correctamente ${prefix}snowwrite texto1|texto2, Ejemplo ${prefix}snowwrite Bot|Samurai`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Usa el comando correctamente ${prefix}marvellogo texto, Ejemplo  ${prefix}marvellogo Samurai bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'meme':
					...
					nzwa.sendMessage(from, nye, image, { caption: 'รศɱนгศı ๖σt ', quoted: mek })
					await limitAdd(sender) 	
					break
				case 'memeindo':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'marcar':
					...
					for (let mem of groupMembers) {
						teks += `รศɱนгศı ๖σt @${mem.jid.split('@')[0]}\n`
				...
				nzwa.updatePresence(from, Presence.composing) 
					...
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
						teks += `รศɱนгศı ๖σt 👤 ${mem.jid.split('@')[0]}@c.us\n`
						members_id.push(mem.jid)
					}
				...
				case 'bc':
					nzwa.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(ind.ownerB())
					if (args.length < 1) return reply('รศɱนгศı ๖σt anuncio dado')
				...
				case 'alay':
                    ...
                    nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'รศɱนгศı ๖σt })
                    await limitAdd(sender)
                    break
                    case 'glitch':
                    gh = body.slice(7)
                    ...
                    nzwa.sendMessage(from, hasil, image, {quoted: mek, caption: 'รศɱนгศı ๖σt '})
                    await limitAdd(sender)
                    break
                     case 'leave':
                    if (!isGroup) return reply(mess.only.group)
	    ...
	    case 'kapankah':
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`Y el texto? Ejemplo ${prefix}logowolf Samurai|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Y el texto? Ejemplo ${prefix}phlogo Samurai|Bot')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}neonlogo Samurai-bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}neonlogo2 SamuraiBot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}lionlogo Samurai|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}jokerlogo SamuraiBot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}shadow SamuraiBot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}burnpaper SamuraiBot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
										...
										case 'coffee':
											var gh = body.slice(7)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}coffee SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
											anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=coffee&text=${teks1}&apikey=BotWeA`, {method: 'get'})
											buffer = await getBuffer(anu.result)
											client.sendMessage(from, buffer, image, {quoted: mek, caption: 'รศɱนгศı ๖σt '})
											await limitAdd(sender)
											break
									case 'lovepaper':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}lovepaper SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'woodblock':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}woodblock SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'qowheart':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}qowheart SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'mutgrass':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}mutgrass SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'undergocean':
											var gh = body.slice(12)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}undergocean SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'woodenboards':
											...
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}woodenboards SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'wolfmetal':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}wolfmetal SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'metalictglow':
											...
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}metalictglow SamuraiBot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case '8bit':
											...
											var teks1 = gh.split("|")[0];
											var teks2 = gh.split("|")[1];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}8bit Samurai|Bot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
				...
				case 'clone':
					...
					break
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}pubglogo Samurai|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}harrypotter SamuraiBot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
			...
			case 'ssweb':
                if (!isRegister) return reply(mess.only.daftarB)
                    if (isBanned) return reply(mess.only.benned)    
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Y el nombre?no tengo una bola de cristal culero')
					teks = body.slice(7)
					reply(ind.wait())
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					if (args.length < 1) return reply('Y el texto?no soy adivino culero')
					teks = body.slice(7)
					if (teks.length > 9) return reply('El texto es largo, de hasta 9 caracteres de largo')
					reply(mess.wait)
                    ...
                    nzwa.sendMessage('573175852207@s.whatsapp.net', optionsp, text, {quoted: mek})
